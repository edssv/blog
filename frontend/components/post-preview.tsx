...
import { absoluteUrlImageFromPayload, cn, formatDate } from '@/lib/utils';

import { Skeleton } from './ui/skeleton';

interface PostPreviewProps extends Pick<Post, 'cover' | 'createdAt' | 'description' | 'id' | 'title'> {
  featured?: boolean;
  cover: Media;
}

export function PostPreview({ cover, createdAt, description, featured = false, id, title }: PostPreviewProps) {
        ...
        <div className={cn({ 'flex flex-col items-center lg:flex-row': featured })}>
          ...
          <div className={cn({ 'w-full basis-full lg:w-auto lg:basis-1/2': featured })}>
            <Image
              ...
              priority={featured}
              sizes='100vw'
              src={absoluteUrlImageFromPayload(cover.url!)}
              width={300}
              className={cn(
...
PostPreview.Skeleton = function PostPreviewSkeleton() {
  return (
